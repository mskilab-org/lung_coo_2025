...
params:
  set_title: "My Title!"
  fileLoc: "/gpfs/commons/groups/imielinski_lab/projects/scLung/db/scLung_RMDFiles/"
---
...
```{r, include=FALSE}
cat(sprintf('<script>document.title = "%s";</script>', params$set_title))
```
...
## Figure 1B
```{r, fig1B, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)

#Load umap coordinates and cell identity information.
umap_coord = read.table(file.path(params$fileLoc, "Fig1/1B_umap_coordinates.csv"),sep=",",header=TRUE)
...
#Consolidate results into percentages per cell group per patient. Count the rows per group, calculate total counts per patient, and calculate percentage.
...
library(MASS)
library(parallel)
res = readRDS(file.path(params$fileLoc, "Fig2/2B_data.rds"))
...
library(MASS)
library(parallel)
res = readRDS(file.path(params$fileLoc, "Fig2/2C_data.rds"))
...
#Load the anneuploidy and sample metadata.
mat.in.2 <- readRDS(file.path(params$fileLoc,'Fig3/3A_mat.in.2.rds'))
col_fun2 <- readRDS(file.path(params$fileLoc,'Fig3/3A_col_fun2.rds'))

#Assign heatmap annotations based on sample metadata.
...
#Generate one heatmap per cluster, add the annotations defined above.
set.seed(90210)
...
#Combine all 4 heatmaps and plot.
ht_list =  ht2+ ht1 + ht3 + ht4 
draw(ht_list, row_title = "Heatmap list", column_title = "Heatmap list")
```

## Figure 3B 
```{r, fig3B, dropBelow=TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)

#For each barplot load the COO prediction output data and plot.
# Distal Lung
fig3B_data <- readRDS(file.path(params$fileLoc,'Fig3/3B_data.rds'))
...
extd_data_3 <- fread(file.path(params$fileLoc,'Fig4/4ABC_extd_data_3.csv'))
ik = 'Never_Smoker'
...
## Figure 4B 
```{r, fig4B, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
## Figure 4C
```{r, fig4C, dropBelow = TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
## Figure 5A
```{r, fig5A, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
library(ggalluvial)
library(wesanderson)

pts_coo_id <- readRDS(file.path(params$fileLoc,'Fig5/5A_pts_coo_id.rds'))
...
ggplot(pts_coo_id,
  ...
  ggtitle("Fig 5A - Origin --> Identity") + coord_flip()
```

## Figure 5B
```{r, fig5B, dropBelow = TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
library(wesanderson)

pts_coo_id_4B <- readRDS(file.path(params$fileLoc,'Fig5/5B_pts_coo_id.rds'))
...
```{r, fig6-lead, dropAboveExp=TRUE, results = "asis", echo = FALSE}
```
...
dt.plot = readRDS(file.path(params$fileLoc,"Fig6/6B_cov_res.rds"))
dt.plot$Cell_Type = dt.plot$celltype
dt.plot$tumor.type = "LUAD"
...
## Figure 6C
```{r, fig6C, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries. 
library(skitools)
#Load file with umap of cancer cells output. This allows for gathering WCM-1A and B cells. 
tmp.plot = readRDS(file.path(params$fileLoc,"Fig6/6CD_tmp_plot_for_umap_fig6_knn6.rds"))
...
tmp_norm = readRDS(file.path(params$fileLoc,"Fig6/Fig6C_anneuploidy_zscore.rds"))
...
## Figure 6D
```{r, fig6D, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries 
library(skitools)

#Load per cell identity labels. Remove cells with no identity.
labels = read.table(file.path(params$fileLoc,"Fig6/6D_combined_emb_for_carc_for_homogenous_sikk_ep_500_nw.csv"),sep=",",header=TRUE)
...
## Figure 6E
```{r, fig6E, dropBelow=TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries
library(skitools)
library(EnhancedVolcano)

#Load file with marker genes.
seu.markers = readRDS(file.path(params$fileLoc,"Fig6/6E_seu_markers.rds")) 
...
expDat = readRDS(file.path(params$fileLoc,"EDF/EDF1/1A_supExp.rds"))
clustDat = readRDS(file.path(params$fileLoc,"EDF/EDF1/1A_supClusters.rds"))
...
tmp.plot = data.table(readRDS(file.path(params$fileLoc,"EDF/EDF1/1BC_seuMetaData.rds")))
tmp.l.norm = readRDS(file.path(params$fileLoc,"EDF/EDF1/1BC_anneuploidy_zscore.rds"))
exp_shannon = readRDS(file.path(params$fileLoc,"EDF/EDF1/1BC_clusterEntropy.rds"))
...
sikkema_meta = read.table(file.path(params$fileLoc,"EDF/EDF2/2_sikk_meta_epi.csv"),header=T,sep=",")
...
```{r, extfig3a, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)

#Load file with cell metadata and identity call results.
normal_labs = fread(file.path(params$fileLoc,"EDF/EDF3/3A_combined_emb_for_normal_for_homogenous_sikk_ep_nw5.csv"))
...
## Extended Data Figure 3B - Left Panel
```{r, extfig3BLeft, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load required libraries.
library(skitools)

#Load GRanges with centroid expression data.
juan.genes.gr.nw = readRDS(file.path(params$fileLoc,"EDF/EDF3/3BLP_genes_gr_LUAD.rds"))
...
## Extended Data Figure 3B - Right Panel
```{r, extfig3BRight, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load required libraries.
library(skitools)

#Load GRanges with centroid expression data.
juan.genes.gr.nw = readRDS(file.path(params$fileLoc,"EDF/EDF3/3BRP_genes_gr_LUSC.rds"))
juan.genes.gr.nw.dt = gr2dt(juan.genes.gr.nw)
...
## Extended Data Figure 3C - Left Panel
```{r, extfig3CLeft, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries
library(skitools)
...
dt1 = readRDS(file.path(params$fileLoc,"EDF/EDF3/3C_DeconstructSigsV3_MutDensity_Patients_SikkCentroids_Vs_Genes_All_Celltypes_PreAvgExprP1.rds"))
dt2 = readRDS(file.path(params$fileLoc,"EDF/EDF3/3C_DeconstructSigsV3_MutDensity_Patients_SikkCentroids_Vs_Genes_All_Celltypes_PreAvgExprP2.rds"))
dt3 = readRDS(file.path(params$fileLoc,"EDF/EDF3/3C_DeconstructSigsV3_MutDensity_Patients_SikkCentroids_Vs_Genes_All_Celltypes_PreAvgExprP3.rds"))
...
luad_pairs = readRDS(file.path(params$fileLoc,"EDF/EDF3/3C_new.pairs.luad.rds"))
...
sikk.cent = readRDS(file.path(params$fileLoc,"EDF/EDF3/3C_epcells.rds"))
...
## Extended Data Figure 3C - Center Panel
```{r, extfig3CCenter, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries
library(skitools)
...
## Extended Data Figure 3C - Right Panel
```{r, extfig5CRight, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries
library(skitools)
...
```{r, extfig3D, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)

# Accuracy for predicting true cell type at different levels 

sims_acc_results <- fread(file.path(params$fileLoc,'EDF/EDF3/3D_luad_sims_results.csv'))
...
```{r, extfig3E, dropBelow = TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
resDT = readRDS(file.path(params$fileLoc,"EDF/EDF3/3E_testRun_withOverlap_Cov.rds"))
...
annotSikk = read.csv(file.path(params$fileLoc,"EDF/EDF3/3E_sikk_meta_epi3.csv"))
...
#Line graph is plotted. Depending on whether both uni and multivariate are plotted use the first line or the second respectively (comment/uncomment).
...
nsclc_col <- readRDS(file.path(params$fileLoc,'EDF/EDF4/4A_nsclc_hm_col.rds'))
hm_lusc <- fread(file.path(params$fileLoc,'EDF/EDF4/4A_hm_lusc.csv'))
...
edf_8_data <- readRDS(file.path(params$fileLoc,'EDF/EDF4/4B_data.rds'))
...
```{r, extfig5-lead, dropAboveExp=TRUE, results = "asis", echo = FALSE}
```
## Extended Data Figure 5A
```{r, extfig5A, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)

pts_coo_id_edf9a <- readRDS(file.path(params$fileLoc,'EDF/EDF5/5A_pts_coo_id.rds'))
...
```{r, extfig5B, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
library(ggsankey)

tmp.plot.luad.df3 <- readRDS(file.path(params$fileLoc,'EDF/EDF5/5B_data.rds'))
...
```{r, extfig5C, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)

edf9 <- readRDS(file.path(params$fileLoc,'EDF/EDF5/5CDEF_data.rds'))
...
```{r, extfig5D, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
```{r, extfig5E, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
```{r, extfig5F, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
```{r, extfig5G, dropBelow = TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
library(skitools)
#Load libraries.
edf5g_a <- readRDS(file.path(params$fileLoc,'EDF/EDF5/5G_dataA.rds'))
edf5g_b <- readRDS(file.path(params$fileLoc,'EDF/EDF5/5G_dataB.rds'))
...
```{r, fig6A, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries
library(skitools)
library(parallel)
library(MASS)
...
```{r, extfig6B, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
...
```{r, extfig6C, dropBelow = TRUE, results = "asis", echo = TRUE, out.width= "100%", fig.align = "center"}
#Load libraries.
library(skitools)
library(RColorBrewer)
tmp.plot = readRDS(file.path(params$fileLoc,"EDF/EDF6/6BC_data.rds"))
