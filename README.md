...
**If you're interested specifically in checking the R notebook with the code and figures, you can find the latest version** [here](https://rawcdn.githack.com/mskilab-org/lung_coo_2025/130dd8f65ddf2427ae7c313e923c27d305f44d17/data/scLung_Figures.html) **.**

# Repository Structure

## Code Files

- The `R` folder contains a total of 10 files. Out of these, the 7 that begin with the prefix `fig` contain code for either 1 of the 6 main figures (`fig1.R` to `fig6.R`), or the extended data figures (`figs_extd.R`). Within these files, the code is divided by panel, is commented, and can be run individually for single panels without running the whole file. The required libraries and files for generating each panel (see below) are loaded individually. 
...
- The `COO_Simulations.R` file contains the code used for simulations for extended data figures 3D and 3E. The code as is will generate simulations for extended data figure 3E, but instructions are included in the code's comments to adjust for generating simulations for extended data figure 3D.
